 ...
 *
 * If `[MSCrashes enableAppNotTerminatingCleanlyDetection]` is enabled, use this on startup to check if the app starts
 * the first time after it was killed by iOS in the previous session.
 *
 * This can happen if it consumed too much memory or the watchdog killed the app because it took too long to startup or
 * blocks the main thread for too long, or other reasons. See Apple documentation:
 ...
 *
 * @warning This property only has a correct value, once `[BITHockeyManager startManager]` was invoked! In addition, it
 * is automatically disabled while a debugger session is active!
 *
