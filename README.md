...
Note that the events must be subscribed to and callbacks must be set before Mobile Center is started.
    ...
    * **User Confirmation:** By default the SDK automatically transmits crash reports to Mobile Center. However, the SDK exposes a callback where you can tell it to await user confirmation before sending any crash reports.
